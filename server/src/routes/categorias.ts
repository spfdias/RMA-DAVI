...
router.get('/', async (_req: Request, res: Response) => {
  const categorias = await queryAll('SELECT * FROM categorias ORDER BY label ASC');
  res.json(categorias);
});
...
router.post('/', async (req: Request, res: Response) => {
  ...
  const existing = await queryOne('SELECT id FROM categorias WHERE value = ?', [slug]);
  if (existing) {
  ...
  res.status(201).json(await queryOne('SELECT * FROM categorias WHERE id = ?', [result.lastInsertRowid]));
});
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  const existing = await queryOne('SELECT * FROM categorias WHERE id = ?', [req.params.id]);
  if (!existing) {
